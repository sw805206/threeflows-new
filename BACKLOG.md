...
| BL-043 | review | page | Verify the unverified rate rows in `assets/tool-overall-tax-estimator.js` against their cited sources. **The compiled half is DONE (PR141).** The 51-jurisdiction sales-tax exemption table was read back against published 2026 compilations: clothing verified in all 51 (Minnesota, New Jersey, Pennsylvania and Vermont exempt; Massachusetts $175, New York $110, Rhode Island $250; all others taxable, and no value changed), and groceries verified in 40 of 51 — the treatment was confirmed everywhere, but no source publishes the combined state-and-local rate on food that the model needs, so the nine reduced-rate states whose figure is derived stay ours. Virginia (1%) and Utah (3%) verify because their combined rates are published; both values were already correct, as was Alabama's 2% state rate. One conflict is recorded rather than resolved: Arkansas reads 0.125% in one 2026 compilation and eliminated-on-1-January-2026 in another. Verification flags are now per field rather than per row. **STILL OPEN:** the fields PORTED from the reference model — the QBI deduction rate, the SALT cap indexing, every state's individual brackets and standard deduction, effective property tax rates, unemployment insurance rates, employer paid-family-leave rates, PTET flags, and the gross receipts / franchise / entity fee regimes — plus the FUTA rate and wage base. Services treatment is deliberately unchecked and does not enter the arithmetic. The two bucket fractions (MIXED_BUCKET_TAXABLE_FRACTION, CLOTHING_SHARE_OF_RETAIL) and the household spend inflation rate are our own assumptions and can never be verified, only replaced. Step 5 reports all of this per row. | 2026-08-24 | |
