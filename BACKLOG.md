...
| BL-030 | close | `.tf-data-table td.tf-fit-you`, `td.tf-fit-assist` and `td.tf-fit-we`, plus the standalone `.tf-fit-you` / `.tf-fit-assist` / `.tf-fit-we` rules, hardcode `#8F1E12`, `#7A5410` and `#175355` — the same three values tokenized as `--tf-wash-*-ink` in PR #93. Point them at the tokens. Deliberately excluded from #93 to keep that refactor's verification intact. NB this row as raised named only `-assist` and `-we`; `#8F1E12` lives solely on `.tf-fit-you`, so all three classes were converted. Recorded at the time only in the STYLE.css v41 changelog and the STYLE.md ratchet | `.tf-data-table td.tf-fit-assist` and `td.tf-fit-we`, plus the standalone `.tf-fit-assist` / `.tf-fit-we` rules, hardcode `#7A5410`, `#175355` and `#8F1E12` — the same three values tokenized as `--tf-wash-*-ink` in PR #93. Point them at the tokens. Deliberately excluded from #93 to keep that refactor's verification intact. Currently recorded only in the STYLE.css v41 changelog and the STYLE.md ratchet | 2026-07-31 | PR97 |
...
| BL-035 | open | STYLE.css's v40 changelog (~lines 80-82, not 53-56 — the v41 and v42 entries were prepended above it) and TWO STYLE.md passages describe the fit-dot pattern with names and hues that never shipped. STYLE.md ~1626-1638 names `.tf-fit-dot-solo` / `-together` / `-no` on teal / plum / stone with three wrong hex pairs; STYLE.md ~1697-1703 — omitted when this row was raised — names `td.tf-fit-solo/-together/-no` and additionally calls `.tf-fit-dot` retired, which it is not (it ships six times across the two legends; v40 retired the dot COLUMN, not the class). Shipped code is `.tf-fit-you` / `-assist` / `-we` on brick / ochre / teal. The ratchet is what the reuse check reads before defining a new pattern, so a wrong entry sends the next reader looking for classes that do not exist. Correct all three records; do not rewrite history beyond making them accurate | STYLE.css v40 changelog (lines 53-56) and the STYLE.md v40 ratchet entry (lines 1626-1638) describe the fit-dot pattern as `.tf-fit-dot-solo` / `-together` / `-no` on teal / plum / stone. Shipped code is `.tf-fit-you` / `-assist` / `-we` on brick / ochre / teal. The ratchet is what the reuse check reads before defining a new pattern, so a wrong entry sends the next reader looking for classes that do not exist. Correct both records; do not rewrite history beyond making them accurate | 2026-08-01 |  |
