...
**§1–§4 — Logo, Color, Typography, Layout & rules — now live in [TOKENS.md](TOKENS.md).** Their numbers are left vacant here rather than reused, so §5 and §6 keep the numbers every cross-reference in the ratchet record, PROCESS.md, BLOG.md and SCOPE.md already points at.
...
`PROCESS.md`, a token name in `SCOPE.md`, a token value in `TOKENS.css` or the
description of that token in `TOKENS.md` — both files change in the **same
...
### Design tokens split out — TOKENS.css + TOKENS.md — 2026-08-14

A PURE MOVE, verified by capture: every `--tf-*` custom property resolved on
`int-stylebook.html` before and after the split is byte-identical (62 properties,
including the three component-scoped ones that resolve empty on `:root`). No
token was added, removed, renamed, reordered or revalued, and no rule changed.
Paired with STYLE.css v059, same commit per SCOPE.md.

- **What moved.** The 121-line `:root` block (STYLE.css lines 782–902) became
  `TOKENS.css`; STYLE.css's "1. Design tokens" section now holds a single
  `@import url('TOKENS.css');` in its place, so the section header still heads
  its content and nothing is orphaned. STYLE.css 4063 → 3944 lines. The import
  sits after the stamp and font comments and before every rule, which is where
  CSS requires it — a style rule above it would make the browser drop it
  silently, and the whole palette with it. A one-line comment directly above the
  `@import` says so, so a later edit into that region cannot disable it by
  accident.
- **Why split at all.** `threeflows-app` needs the same brand. SCOPE.md §3 makes
  TOKENS.css and TOKENS.md the sync manifest; the alternative was re-declaring
  the palette by hand in the app, which is precisely the two-places-drift failure
  this section exists to prevent.
- **TOKENS.css and TOKENS.md are SCOPE.md §3 manifest files, and the sibling is
  NOT yet bootstrapped.** Both exist here and are absent from `threeflows-app`,
  which §3 rule 3 classes as "absent on one side → bootstrap: copy across, no
  comparison". That copy is deliberately not part of this commit. Until it runs
  the manifest is live but asymmetric, so the next task touching either file
  bootstraps first, before authoring anything on top of it.
- **STYLE.md §1–§4 moved to TOKENS.md and their numbers were NOT reused.** §5 and
  §6 keep their numbers and §1–§4 are left vacant. Renumbering would have
  silently broken every §-reference in this ~2,900-line record and in
  PROCESS.md, BLOG.md and SCOPE.md — and broken them in the worst way, since each
  would still resolve to *a* heading, just the wrong one.
- **The section separator was deliberately not carried across.** TOKENS.md takes
  STYLE.md lines 9–113; STYLE.md deletes 9–116. The asymmetry is the point:
  moving the trailing `---` would end TOKENS.md on a dangling rule, and keeping
  it in STYLE.md would stack two `---` before §5.
- **int-stylebook.html's header stamps now quote the LINE-1 stamps**, not the
  internal history counters they quoted before: STYLE.md v064 / STYLE.css v059 /
  TOKENS.md v001 / TOKENS.css v001. Recorded here so it is not re-litigated. The
  `paired with STYLE.css vNNN` convention tracks line 1 — the proof is this
  record's own STYLE.css v56 entry citing "STYLE.md v060", a number the line-5
  running summary (frozen at v038) has never held and never will. Line 1 is also
  the only counter guaranteed to bump on every substantive change: STYLE.css v057
  and v058 each bumped it without adding an internal entry, which is how the
  internal counter drifted to v56 against a v058 stamp. And TOKENS.css /
  TOKENS.md have no second counter to quote. PROCESS.md §4 names no counter, so
  this disambiguates it rather than contradicting it; amending §4's wording is a
  separate follow-up and is NOT part of this commit.
- **STYLE.css's internal history counter was deliberately NOT bumped.** It stays
  at v56, following the v057/v058 precedent — the change is recorded here, in §6,
  rather than in two places. Restarting the internal counter would widen the very
  drift this section warns about.
- **STYLE.md's line-5 running summary is untouched.** It has been frozen at v038
