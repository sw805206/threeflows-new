...
The **core palette** is the logo's four inks; charts and tags may draw on the data palette below. Two named surface exceptions — `--tf-white` (header chrome) and `--tf-footer-ground` — are scoped, not relaxations.
...
**Title Case in headings** — Headings (h1/h2/h3) use conventional (Chicago-style) Title Case: capitalize the first and last word and all principal words; lowercase articles (a, an, the), coordinating conjunctions (and, but, or), and prepositions regardless of length (to, from, of, in, on, with…). Title case is AUTHORED IN THE MARKUP — it is NOT applied via CSS `text-transform`, which would capitalize every word including function words and cannot lowercase them. Applies to all headings unless a specific component rule says otherwise (e.g. kickers/meta, which are uppercased via their own class).
