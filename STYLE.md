...
- **Footer ground `--tf-footer-ground` — footer only. RATIFIED.** `color-mix(in srgb, var(--tf-ink) 75%, var(--tf-stone))` ≈ `#4A4540` — a dark warm stone, lighter than ink, so the footer reads as its own band when an ink `.tf-cta` sits directly above it, separated on tone alone. A derived mix per the cream idiom, not a raw hue; scoped to one surface. Footer links: `color-mix(in srgb, var(--tf-brick-on-ink) 45%, var(--tf-paper))` ≈ `#EBA89E` (~4.8:1 measured). See §6.
